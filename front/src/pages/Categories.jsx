...
import { ShoppingBag, Search, Filter, Star, TrendingUp, Sparkles } from 'lucide-react';
...
const Categories = () => {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => setDebouncedSearch(searchTerm.trim()), 300);
  ...
  const filteredProducts = useMemo(() => {
    if (!allProducts) return [];
    let list = selectedCategory === 'All' ? allProducts : allProducts.filter((p) => p.category === selectedCategory);
    if (debouncedSearch) {
  ...
  const handleAddToCart = useCallback(
    async (id) => {
      ...
      try {
        ...
        setTimeout(() => setAddingIds((s) => s.filter((x) => x !== id)), 700);
      } catch (err) {
  ...
  return (
    <>
      ...
      <div className="relative w-full min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 pt-6 pb-16 px-4 sm:px-6 lg:px-8 overflow-hidden">

        {/* Background decorations */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-20 right-10 w-72 h-72 bg-gradient-to-br from-blue-400/20 to-indigo-400/20 rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-20 left-10 w-96 h-96 bg-gradient-to-tr from-cyan-400/20 to-purple-400/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          <div className="mb-12 space-y-6">
            <div className="max-w-2xl mx-auto">
              <div className="relative group">
                ...
                <div className="relative flex items-center bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
                  ...
                  />
                  {searchTerm && (
                    <button onClick={() => setSearchTerm('')} className="absolute left-6 text-gray-400 hover:text-gray-600 transition-colors duration-200">
            ...
            <div className="relative">
              <div className="flex items-center justify-center gap-4 mb-4">
                ...
                <h3 className="text-lg font-bold text-gray-800">تصفية حسب الفئة</h3>
              </div>
              {isLoading ? (
                <div className="flex flex-wrap justify-center gap-3">
                  {Array.from({ length: 5 }).map((_, i) => <CategoryButtonSkeleton key={i} />)}
                </div>
              ) : (
                <div className="flex flex-wrap justify-center gap-3">
                  {categories.map((cat) => (
                    ...
                    >
                      <span className="relative flex items-center gap-2">
                        {cat === 'All' ? <><TrendingUp className="w-5 h-5" />الكل</> : cat}
                      </span>
                    </button>
                  ))}
                </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 space-y-6">
              ...
              <h3 className="text-2xl font-bold text-gray-800">لا توجد منتجات</h3>
              <p className="text-gray-600 text-center max-w-md">جرب البحث بكلمات مختلفة أو اختر فئة أخرى</p>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between mb-8">
                <p className="text-lg text-gray-600 font-medium">
                  <span className="text-blue-600 font-bold text-xl">{filteredProducts.length}</span> منتج متاح
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {filteredProducts.map((product, index) => {
                  ...
                  return (
                    <div
                      key={product._id}
                      className={`group relative bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl hover:shadow-2xl overflow-hidden transition-all duration-500 hover:-translate-y-2 border border-gray-100 ${animateProducts ? 'animate-scaleIn opacity-100' : 'opacity-0'}`}
                      style={{ animationDelay: `${index * 50}ms` }}
                    >
                      {/* Image */}
                      <div
                      ...
                      >
                        ...
                        <LazyImage src={product.image} alt={product.name} className="w-full h-full object-contain p-6 group-hover:scale-110 transition-transform duration-500" />
                        <div className="absolute inset-0 bg-gradient-to-t from-blue-600/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6">
                          <span className="text-white font-bold text-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">عرض التفاصيل</span>
                        </div>
                      </div>
                      ...
                      <div className="p-6 space-y-4">
                        <div className="space-y-2">
                          <h3 className="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors duration-300 line-clamp-1">{product.name}</h3>
                          <p className="text-gray-600 text-sm leading-relaxed line-clamp-2 h-10">{product.description}</p>
                        </div>
                        ...
                        {product.ratings?.average > 0 && (
                          <div className="flex items-center gap-2">
                            <div className="flex items-center gap-1">
                              {[...Array(5)].map((_, i) => (
                                <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.ratings.average) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />
                              ))}
                            </div>
                            <span className="text-sm text-gray-600 font-medium">({product.ratings.count})</span>
                          </div>
                        )}

                        <div className="flex items-baseline gap-2">
                          <p className="text-3xl font-black bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">{formatEGP(product.price)}</p>
                          {hasDiscount && <span className="text-sm text-gray-400 line-through">{formatEGP(product.originalPrice)}</span>}
                        </div>
                        ...
                        >
                          {isAdding ? (
                            <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /><span>جاري الإضافة...</span></>
                          ) : (
                            <><ShoppingBag className="w-5 h-5" strokeWidth={2.5} /><span>أضف للسلة</span></>
                          )}
                        </button>
                      </div>
