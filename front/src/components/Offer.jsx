...
const Offer = () => {
  ...
  const TOAST_DURATION = 1800; // مدة ظهور الرسالة
      ...
      <section className="relative w-full bg-gray-50 text-gray-800 py-24 px-6 sm:px-10">
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
            ...
            <div
              className="inline-flex items-center gap-4 bg-white/90 px-4 py-3 rounded-full shadow-md"
              role="status"
              aria-live="polite"
            >
              ...
              <div className="flex gap-3">
                {[
                  ...
                  <div key={label} className="text-center">
                    <div className="bg-cyan-50 text-cyan-600 font-bold rounded-lg px-3 py-2 w-16">
                      {pad(val)}
                    </div>
          ...
          {products === null ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
              {Array.from({ length: 8 }).map((_, i) => (
                <div
                  key={i}
                  className="animate-pulse bg-white rounded-3xl shadow-lg p-4 h-96"
                  aria-hidden
                />
              ))}
            </div>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onAdd(p._id);
                          }} /* prevent navigation when clicking Add */
                          disabled={loading}
                          aria-busy={loading}
                          aria-label={
                            loading
