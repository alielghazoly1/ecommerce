              ...
              <div className="flex gap-2">
                {[['أيام', days], ['ساعات', hours], ['دقائق', minutes], ['ثواني', seconds]].map(([label, val]) => (
                  <div key={label} className="text-center">
                    <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 text-white font-bold rounded-lg px-2.5 py-1.5 min-w-[3rem] shadow-md">{pad(val)}</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative w-full h-64 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
                      <LazyImage src={p.image} alt={p.name} className="object-contain w-full h-full p-4 transition-transform duration-500 group-hover:scale-110" />
                    </div>
                    <div className="p-5">
                      <h3 className="text-lg font-bold text-gray-900 truncate mb-2">{p.name}</h3>
                      {p.description && <p className="text-gray-600 text-sm line-clamp-2 mb-4 min-h-[40px]">{p.description}</p>}
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          {hasDiscount ? (
                            <div className="flex flex-col gap-1">
                              <span className="text-xl font-bold text-cyan-600">{formatEGP(p.price)}</span>
                              <span className="text-xs line-through text-gray-400">{formatEGP(p.originalPrice)}</span>
                            </div>
                          ) : (
                            <span className="text-xl font-bold text-cyan-600">{formatEGP(p.price)}</span>
                          )}
